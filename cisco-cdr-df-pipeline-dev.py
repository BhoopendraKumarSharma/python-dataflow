...
class ProcessFileDoFn(beam.DoFn):
    ...
    def process(self, element):
        ...
        try:
            ...
            return [(orig_file_path, new_file_path, file_content, None)]
        ...
        if processed_data:
            ...
            try:
                ...
                if load_job.errors:
                    ...
                    if file_content:
                        # Write data to GCS
                        output_bucket = "gs://cisco-cdr-dev-output_failed_files"
                    ...
                    if file_content:
                        # Write data to GCS
                        output_bucket = "gs://cisco-cdr-dev-output"
                        output_path = output_bucket + '/' + file_path
                        with beam.io.gcsio.GcsIO().open(output_path, "w") as f:
                            f.write(file_content.encode('utf-8'))
                            # To delete the file from source location; it will fail if permission is not granted for delete
                ...
                logging.error(f"Error writing to BigQuery for file {orig_file_path}: {e}")
                if file_content:
                    # Write data to GCS
                    output_bucket = "gs://cisco-cdr-dev-output_failed_files"
                    output_path = output_bucket + '/' + file_path
            ...
            logging.error(f"Error in reading/writing to BigQuery for file {orig_file_path}")
            if file_content:
                # Write data to GCS
                output_bucket = "gs://cisco-cdr-dev-output_failed_files"
                output_path = output_bucket + '/' + file_path
...
def run(argv=None):
    ...
    input_bucket = "gs://cisco-cdr-dataflow-pipeline-dev-input"
    output_bucket = "gs://cisco-cdr-dev-output"
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        input_file_patterns = p | "Match All New Files" >> fileio.MatchContinuously(f"gs://cisco-cdr-dataflow-pipeline-dev-input/cdr*",
                                                                                    interval=60.0)
        input_files = (
                input_file_patterns
