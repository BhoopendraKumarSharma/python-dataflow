...
PROJECT_ID = "wmt-cc-datasphere-dev"
input_path = "gs://cisco-cdr-dataflow-pipeline-dev-input/cdr*"
output_table = "wmt-cc-datasphere-dev.cisco_cdr.cisco-cdr-processed-data"
output_success_bucket = "gs://cisco-cdr-dev-output"
output_failure_bucket = "gs://cisco-cdr-dev-output_failed_files"
...
def run(argv=None):
    ...
    pipeline_options.view_as(
        beam.options.pipeline_options.GoogleCloudOptions).job_name = 'cisco-cdr-df-pipeline-dev-v001'
    pipeline_options.view_as(
        beam.options.pipeline_options.GoogleCloudOptions).staging_location = 'gs://cisco-cdr-df-dev-processing/staging/'
    pipeline_options.view_as(
        beam.options.pipeline_options.GoogleCloudOptions).temp_location = 'gs://cisco-cdr-df-dev-processing/tmp1/'
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        # Extract the data fields for writing the file to GCS
        extracted_data_content_for_gcs = (
                new_files_content_results
                | "Extract Data for GCS" >> beam.Map(lambda element: (element['orig_file_path'], element['new_file_path'], element['content'], element['status']))
        )

        # remove duplicates
        unique_data_content_for_gcs = (
                extracted_data_content_for_gcs
                | "Unique Data for GCS" >> beam.Distinct()
        )
