...
class WriteToPubSubDoFn(beam.DoFn):
    """
    ...
    def __init__(self):
        ...
        """
        self.publisher = None
        self.topic_path = "projects/{}/topics/{}".format(PROJECT_ID, TOPIC_NAME)
    ...
    def process(self, element):
        ...
        if processed_data:
            for record in processed_data:
                self.publisher.publish(self.topic_path, json.dumps(record).encode("utf-8"))
            logging.info(f"Published {len(processed_data)} rows to PubSub Topic {self.topic_path}")
            if file_content:
                # Write data to GCS
...
def run(argv=None):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        input_files = (
                ...
                | "Write To PubSub And GCS" >> beam.ParDo(WriteToPubSubDoFn()).with_outputs("gcs_output",
                                                                                                main="bq_output")
        )
