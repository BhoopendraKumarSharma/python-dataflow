...
class WriteToBigQueryDoFn(beam.DoFn):
    ...
    Returns:
        A dictionary containing the file_path, file_content, and processed_data.
    """
    ...
    def process(self, element):
        ...
        if processed_data:
            self.batch_data.extend(processed_data)

        return [(orig_file_path, file_path, file_content, processed_data)]

    def finish_bundle(self):
        """
        Writes the collected batch data to BigQuery.
        """
        if self.batch_data:
            ...
            load_job = self.client.load_table_from_json(self.batch_data, table, job_config=job_config)
            load_job.result()  # Waits for the job to complete.
            if load_job.errors:
                logging.error(f"Failed to insert rows into BigQuery table {self.table_ref}: {load_job.errors}")
            else:
                logging.info(f"Inserted {len(self.batch_data)} rows into BigQuery table {self.table_ref}")
                self.batch_data = []  # Clear the batch data after writing


class MoveAndDeleteFileDoFn(beam.DoFn):
    """
    A DoFn that moves files to the output bucket and deletes files from the original path.
    ...
    def process(self, element):
        """
        Moves files to the output bucket and deletes files from the original path.
        ...
        # Move the file to the output bucket
...
def run(argv=None):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        input_files = (
                ...
                | "Move And Delete Files" >> beam.ParDo(MoveAndDeleteFileDoFn())
        )
