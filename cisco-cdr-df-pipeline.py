        ...
        if processed_data:
            ...
            try:
                ...
                if load_job.errors:
                    ...
                    if file_content:
                        # Write data to GCS
                        output_bucket = "gs://cisco-cdr-output_failed_files"
                    ...
                    if file_content:
                        # Write data to GCS
                        output_bucket = "gs://cisco-cdr-output"
                        output_path = output_bucket + '/' + file_path
                        with beam.io.gcsio.GcsIO().open(output_path, "w") as f:
                            f.write(file_content.encode('utf-8'))
                            # To delete the file from source location; it will fail if permission is not granted for delete
...
def run(argv=None):
    ...
    input_bucket = "gs://cucm_cdr_archive/cdr"
    output_bucket = "gs://cisco-cdr-output"
    ...
    pipeline_options.view_as(
        beam.options.pipeline_options.GoogleCloudOptions).job_name = 'cisco-cdr-df-pipeline-v001'
    pipeline_options.view_as(
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        input_file_patterns = p | "Match All New Files" >> fileio.MatchContinuously(f"gs://cucm_cdr_archive/cdr/cdr*",
                                                                                    interval=60.0)
        input_files = (
                input_file_patterns
